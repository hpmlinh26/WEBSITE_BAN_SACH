// admin.js - quản trị sản phẩm, danh mục, tài khoản, đơn hàng, voucher và phản hồi.
(function () {
  ...
  function fillOrderProductSelect() {
    ...
    if (!select) return;
    select.innerHTML = `<option value="">Chọn sản phẩm</option>` + state.products.map(p =>
      `<option value="${p.id}">${escapeHtml(p.name || p.title)} - ${money(p.price)}</option>`
    ).join("");
  }
  ...
  function renderCategories() {
    ...
    wrap.innerHTML = state.categories.map(c => `
      ...
      </article>`).join("") || `<p class="empty-admin">Chưa có danh mục.</p>`;
  }
  ...
  function renderOrders() {
    ...
    tbody.innerHTML = state.orders.map(o => `
      ...
      </tr>`).join("") || `<tr><td colspan="6">Chưa có đơn hàng.</td></tr>`;
    renderMetrics();
  ...
  function renderFeedbacks() {
    ...
    if (!wrap) return;
    wrap.innerHTML = state.feedbacks.map(f => `
      ...
      </article>`).join("") || `<div class="empty-admin">Chưa có phản hồi nào.</div>`;
  }
  ...
  function renderUsers() {
    ...
    tbody.innerHTML = state.users.map(u => `
      ...
      </tr>`).join("") || `<tr><td colspan="6">Chưa có tài khoản.</td></tr>`;
  }
  ...
  function renderVouchers() {
    ...
    tbody.innerHTML = state.vouchers.map(v => `
      ...
      </tr>`).join("") || `<tr><td colspan="6">Chưa có voucher.</td></tr>`;
  }
  ...
  window.handleSaveCategory = async function (event) {
    ...
    const payload = { name, slug: document.getElementById("categorySlug").value.trim() || slugify(name), image: normalizeImage(document.getElementById("categoryImage").value) };
    try { await api(id ? `/categories/${id}` : "/categories", { method: id ? "PUT" : "POST", body: JSON.stringify(payload) }); closeModal(); await reloadAll(); alert("Đã lưu danh mục."); }
    catch (error) { alert(error.message); }
  };
  window.deleteCategory = async function (id) {
    if (!confirm("Xóa danh mục này? Sản phẩm thuộc danh mục sẽ chuyển thành chưa phân loại.")) return;
    try { await api(`/categories/${id}`, { method: "DELETE" }); await reloadAll(); alert("Đã xóa danh mục."); }
    catch (error) { alert(error.message); }
  };
  ...
  window.deleteProduct = async function (id) {
    ...
    try { await api(`/products/${id}`, { method: "DELETE" }); await reloadAll(); alert("Đã xóa sản phẩm."); }
    catch (error) { alert(error.message); }
  };
  ...
  window.handleCreateOrder = async function (event) {
    ...
    try { await api("/orders", { method: "POST", body: JSON.stringify(payload) }); closeModal(); await reloadAll(); alert("Đã tạo đơn hàng."); }
    catch (error) { alert(error.message); }
  };
  ...
  window.handleUpdateOrder = async function (event) {
    ...
    try { await api(`/orders/${id}/status`, { method: "PATCH", body: JSON.stringify({ status }) }); closeModal(); await reloadAll(); alert("Đã cập nhật trạng thái đơn hàng."); }
    catch (error) { alert(error.message); }
  };
  window.deleteOrder = async function (id) {
    if (!confirm("Xóa đơn hàng này?")) return;
    try { await api(`/orders/${id}`, { method: "DELETE" }); await reloadAll(); alert("Đã xóa đơn hàng."); }
    catch (error) { alert(error.message); }
  };
  ...
  window.handleSaveUser = async function (event) {
    ...
    try { await api(id ? `/users/${id}` : "/users", { method: id ? "PUT" : "POST", body: JSON.stringify(payload) }); closeModal(); await reloadAll(); alert("Đã lưu tài khoản."); }
    catch (error) { alert(error.message); }
  };
  window.deleteUser = async function (id) {
    if (!confirm("Xóa tài khoản này?")) return;
    try { await api(`/users/${id}`, { method: "DELETE" }); await reloadAll(); alert("Đã xóa tài khoản."); }
    catch (error) { alert(error.message); }
  };
  ...
  window.handleSaveVoucher = async function (event) {
    ...
    try { await api(id ? `/vouchers/${id}` : "/vouchers", { method: id ? "PUT" : "POST", body: JSON.stringify(payload) }); closeModal(); await reloadAll(); alert("Đã lưu voucher."); }
    catch (error) { alert(error.message); }
  };
  window.deleteVoucher = async function (id) {
    if (!confirm("Xóa voucher này?")) return;
    try { await api(`/vouchers/${id}`, { method: "DELETE" }); await reloadAll(); alert("Đã xóa voucher."); }
    catch (error) { alert(error.message); }
  };

  window.markFeedback = async function(id, status) {
    try { await api(`/feedbacks/${id}/status`, { method: "PATCH", body: JSON.stringify({ status }) }); await reloadAll(); }
    catch(error) { alert(error.message); }
  };
