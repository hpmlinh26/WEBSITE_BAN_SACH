...
function flyToCart(sourceEl, product) {
  ...
  const startImg =
    sourceEl.closest('.product-item, .product-detail-card, .order-item')?.querySelector('img') ||
...
function setupSearchSuggest(box, input, submit) {
  ...
  const render = () => {
    ...
    const list = appProducts
      .filter((p) => normalizeText([p.name, p.author, p.categoryName].join(' ')).includes(keyword))
      .slice(0, 6);
    panel.innerHTML = list.length
      ...
      <a class="search-suggest-item" href="product-detail.html?id=${p.id}">
        ...
        <div><strong>${escapeHtml(p.name)}</strong><span>${money(p.price)} • ${escapeHtml(p.author || 'MOT Store')}</span></div>
          ...
          .join('')
      : `<div class="search-suggest-empty">Không tìm thấy sản phẩm phù hợp.</div>`;
...
function initProductsPage() {
  ...
  if (categorySelect) {
    categorySelect.innerHTML =
      ...
      appCategories.map((c) => `<option value="${c.slug}">${escapeHtml(c.name)}</option>`).join('');
  ...
  function getFilteredProducts() {
    ...
    const buildList = (ignoreCategory = false) =>
      appProducts.filter((p) => {
  ...
  function renderSuggestions() {
    ...
    qsa('button', suggestBox).forEach((btn) =>
      btn.addEventListener('click', () => {
  ...
  document.addEventListener('click', (e) => {
    if (!e.target.closest('.product-search-wrap')) suggestBox?.classList.remove('show');
  });
...
function initProductDetailPage() {
  ...
  if (related) {
    related.innerHTML = appProducts
      .filter((p) => p.id !== product.id)
      .slice(0, 5)
      .map((p) => productCard(p, { compact: true }))
      .join('');
